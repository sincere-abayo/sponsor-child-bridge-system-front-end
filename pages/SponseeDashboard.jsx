    ...
    <Layout>
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl shadow-sm border p-6 mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Welcome, Sponsee!</h1>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              <div className="text-sm text-gray-500 mb-1">Current Sponsors</div>
              <div className="text-3xl font-bold text-blue-700">{stats ? stats.currentSponsors : '--'}</div>
              {statsError && <div className="text-xs text-red-500 mt-1">{statsError}</div>}
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-sm text-gray-500 mb-1">Support Received</div>
              <div className="text-3xl font-bold text-green-700">{stats ? formatCurrency(stats.supportReceived) : '--'}</div>
              {statsError && <div className="text-xs text-red-500 mt-1">{statsError}</div>}
            </div>
                ...
                <ul className="divide-y divide-gray-200">
                  {sponsorships.slice(0, 5).map(s => (
                    <li key={s.id} className="py-2 flex justify-between items-center">
                      <div>
                        <div className="font-medium text-gray-900">{s.sponsor?.name || 'Sponsor'}</div>
